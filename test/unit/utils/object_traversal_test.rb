...
describe ObjectTraverser do
  ...
  it "returns values from a hash" do
    subject.extract_value(%w{hash2 hash1string1}, sample_data).must_equal("value3")
    subject.extract_value(%w{hash2 hash1number1}, sample_data).must_equal(123)
  end

  it "returns values from a nested hash" do
    subject.extract_value(%w{hash2 hash1subhash key1}, sample_data).must_equal(1)
    subject.extract_value(%w{hash2 hash1subhash key2}, sample_data).must_equal(2)
  end
  ...
  it "returns values from the array using methods" do
    subject.extract_value(%w{array1 first}, sample_data).must_equal("word1")
    subject.extract_value(%w{array1 last}, sample_data).must_equal("word3")
  end

  it "returns nil when fetching from an array when it does not match a method" do
    subject.extract_value(%w{array1 not_a_valid_method}, sample_data).must_be_nil
  end
  ...
  it "returns values from a nested hash within an array, accessing the array using methods" do
    subject.extract_value(%w{array2 last array1hashkey1}, sample_data).must_equal(1)
    subject.extract_value(%w{array2 last array1hashkey2}, sample_data).must_equal(2)
  end
