...
describe "Inspec::Resources::Elasticsearch" do
  ...
  it "returns a cluster name" do
    _(resource.cluster_name).must_equal %w{foo foo}
  end
  ...
  it "returns node names" do
    _(resource.node_name).must_equal %w{_L4HxbL N1XoIhE}
  end

  it "returns node IDs" do
    _(resource.node_id).must_equal %w{_L4HxbLvQuWJdD5ejUgt8A N1XoIhE6RWGAMTS1fSdsLg}
  end
  ...
  it "returns role information" do
    _(resource.roles).must_equal [%w{master data ingest}, %w{master data ingest}]
  end
